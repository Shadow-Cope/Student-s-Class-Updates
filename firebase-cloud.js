...
(function () {
  ...
  function memberProfile() {
    ...
    return { email: p.email || Cloud.user.email || "guest", firstName: p.firstName || "", lastName: p.lastName || "", role: p.role || "student", pfp: p.pfp || null };
  }
  ...
  Cloud.accountCardHTML = function () {
    ...
    const u = Cloud.user;
    return `<div class="card" style="margin-bottom:16px"><h3>☁ ${t("cloudAcc")}</h3>
      <dl class="kv"><dt>Email</dt><dd>${esc(u.email || "guest")}</dd><dt>Status</dt><dd>● ${t("signedInCloud")}</dd></dl>
  ...
  Cloud.viewClass = function () {
    ...
    <div class="tabs"><button class="tab ${tab === "stream" ? "active" : ""}" data-cc-tab="stream">Stream</button><button class="tab ${tab === "classwork" ? "active" : ""}" data-cc-tab="classwork">Classwork</button><button class="tab ${tab === "people" ? "active" : ""}" data-cc-tab="people">People</button></div>
    ${body}`;
  };
